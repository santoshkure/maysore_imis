...
wardName = parameters.wardName;
code = null;
List<EntityCondition> andExprs = new LinkedList<EntityCondition>();
			EntityCondition mainCond = null;
 		    if(UtilValidate.isNotEmpty(zoneId)) {
                    andExprs.add(EntityCondition.makeCondition("zoneId",
                        EntityOperator.EQUALS,zoneId));

                }		
  				
				if(UtilValidate.isNotEmpty(wardName)) {
					andExprs.add(EntityCondition.makeCondition("wardName",
...
listSearch = from("WardMaster").where(mainCond).orderBy("zoneId").cursorScrollInsensitive().cache(true).queryIterator()
allOfficeWardListed = listSearch.getPartialList(lowIndex, highIndex - lowIndex + 1)
//print("~~~~~~~~~~~~~~~~~allOfficeWardListed~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+allOfficeWardListed);
...
context.allOfficeWardListed = allOfficeWardListed
