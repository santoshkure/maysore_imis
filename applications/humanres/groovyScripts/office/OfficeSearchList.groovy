...
officeType = parameters.officeType;
...
if(UtilValidate.isNotEmpty(officeName)) {
					andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("officeSiteName"),
						EntityOperator.LIKE, EntityFunction.UPPER("%" + officeName + "%")));
				}
				
				if(UtilValidate.isNotEmpty(officeType)) {
					andExprs.add(EntityCondition.makeCondition("groupName",
						EntityOperator.EQUALS,officeType));
				}
...
listIt = from("OfficeDetail").where(mainCond).orderBy("partyId").cursorScrollInsensitive().cache(true).queryIterator()
allOfficeList = listIt.getPartialList(lowIndex, highIndex - lowIndex + 1)
//print("~~~~~~~~~~~~~~~~~allOfficeList~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+listIt);
...
context.allOfficeList = allOfficeList
