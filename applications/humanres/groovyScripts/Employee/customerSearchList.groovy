/*
 * added by shubham malviya for Customer Searching
...
List<EntityCondition> andExprs = new LinkedList<EntityCondition>();
       ...
       if(UtilValidate.isNotEmpty(nameOfCustomer)) {
				...
				if(UtilValidate.isNotEmpty(customerId)) {
					...
					andExprs.add(EntityCondition.makeCondition(EntityOperator.OR,(EntityCondition.makeCondition("actionStatus",
						EntityOperator.EQUALS,"Approve")),(EntityCondition.makeCondition("actionStatus",
						EntityOperator.EQUALS,"Disable"))));
...
customerList = from("consumerRegistrationDetails").where(mainCond).orderBy("customerId").cursorScrollInsensitive().cache(true).queryIterator()

searchCustomerList = customerList.getPartialList(lowIndex, highIndex - lowIndex + 1)
...
context.searchCustomerList = searchCustomerList
