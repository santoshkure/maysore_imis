...
ApplicationTypeKan = parameters.ApplicationTypeKan;
code = null;
List<EntityCondition> andExprs = new LinkedList<EntityCondition>();
			EntityCondition mainCond = null;
 		    if(UtilValidate.isNotEmpty(applicationType)) {
				...
				if(UtilValidate.isNotEmpty(ApplicationTypeKan)) {
					andExprs.add(EntityCondition.makeCondition("ApplicationTypeKan",
						EntityOperator.EQUALS,ApplicationTypeKan));
				}
...
listSearch = from("applicationTypeMaster").where(mainCond).orderBy("applicationTypeeId").cursorScrollInsensitive().cache(true).queryIterator()
applicationMasterListed = listSearch.getPartialList(lowIndex, highIndex - lowIndex + 1)
//print("~~~~~~~~~~~~~~~~~applicationMasterListed~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+applicationMasterListed);
...
context.applicationMasterListed = applicationMasterListed
