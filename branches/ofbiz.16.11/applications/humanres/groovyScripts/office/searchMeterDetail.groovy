 ...
 * under the License.
 * Adde by Anubha for search meter details
 */
...
List<EntityCondition> andExprs = new LinkedList<EntityCondition>();
			EntityCondition mainCond = null;
 		    ...
 		    if(UtilValidate.isNotEmpty(meterNo)) {
                    andExprs.add(EntityCondition.makeCondition("meterNo",
                        EntityOperator.EQUALS,meterNo));
...
listSearch = from("newMeterRegistration").where(mainCond).orderBy("meterId").cursorScrollInsensitive().cache(true).queryIterator()
materDetailList = listSearch.getPartialList(lowIndex, highIndex - lowIndex + 1)
//print("~~~~~~~~~~~~~~~~~materDetailList~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+materDetailList);
...
context.materDetailList = materDetailList
