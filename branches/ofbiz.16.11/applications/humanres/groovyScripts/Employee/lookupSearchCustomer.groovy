...
List<EntityCondition> andExprs = new LinkedList<EntityCondition>();
       ...
       if(UtilValidate.isNotEmpty(nameOfCustomer)) {
				...
				if(UtilValidate.isNotEmpty(customerId)) {
					...
					andExprs.add(EntityCondition.makeCondition(EntityCondition.makeCondition("actionStatus",
...
customerLookupList = from("consumerRegistrationDetails").where(mainCond).orderBy("customerId").cursorScrollInsensitive().cache(true).queryIterator()

lookupSearchList = customerLookupList.getPartialList(lowIndex, highIndex - lowIndex + 1)

listSize = customerLookupList.getResultsSizeAfterPartialList()
...
customerLookupList.close()




context.lookupSearchList = lookupSearchList
