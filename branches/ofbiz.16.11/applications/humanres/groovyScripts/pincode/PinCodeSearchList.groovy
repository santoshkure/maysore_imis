...
//					andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("pinCode"),
//						EntityOperator.LIKE, EntityFunction.UPPER("%" + pinCode + "%")));
//				}


				if(UtilValidate.isNotEmpty(pinCode)) {
					andExprs.add(EntityCondition.makeCondition("pinCode",
						EntityOperator.EQUALS,pinCode));
				}
...
listIt = from("pinCodeMaster").where(mainCond).orderBy("pinCodeId").cursorScrollInsensitive().cache(true).queryIterator()
pinCodeMasterList = listIt.getPartialList(lowIndex, highIndex - lowIndex + 1)
//print("~~~~~~~~~~~~~~~~~pinCodeMasterList~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+pinCodeMasterList);
...
context.pinCodeMasterList = pinCodeMasterList
